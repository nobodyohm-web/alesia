/**
 * LangSmith Evaluation Runner for Alesia
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps Alesia agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `alesia-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'alesia-finance-eval';
    ...
    const experimentName = `alesia-eval-${Date.now().toString(36)}`;
