...
function debugLog(msg: string) {
  try {
    const logDir = alesiaPath('debug', 'logs');
    const logPath = alesiaPath('debug', 'logs', 'gateway-outbound.log');
...
function getActive(accountId?: string): ActiveListener {
  ...
  if (!first) {
    throw new Error('No active WhatsApp listener. Run alesia gateway run.');
  }
  return first;
}
