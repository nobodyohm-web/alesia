...
const DEFAULT_GATEWAY_PATH = alesiaPath('gateway.json');
...
export function getGatewayConfigPath(overridePath?: string): string {
  return overridePath ?? process.env.ALESIA_GATEWAY_CONFIG ?? DEFAULT_GATEWAY_PATH;
}
...
export function resolveWhatsAppAccount(
  ...
  const authDir = account.authDir ?? alesiaPath('credentials', 'whatsapp', accountId);
